...
function runAnalysis(pathString: string, fileString: string, line: number, column: number, m: number) {
  ...
  return new Promise((res: (val: [Set<string>, Set<any>, number]) => void, rej) => {
    ...
    worker.on('exit', (code) => {
      if (code !== 0) {
        console.error(`Abnormal exit ${code}`);
      }
      rej(code);
    })
...
//   printResults(runAnalysis('../../examples/dyrectorio/web/crux', './src/app/notification/notification.service.ts', 82, 26, 3))
// }
...
async function printResults(resultsPromise: Promise<[number, any, any]>) {
  ...
  for (const target of targets) {
    ...
    for (const m of precisions) {
      try {
        ...
        console.warn(`${target.name} ${m}: error ${message.substring(0, 50)}`)
      }
    }
  }
}
